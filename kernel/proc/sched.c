...
{       
        KASSERT(q != NULL);
        if(curthr->kt_cancelled != 1){ // not cancel yet
...
sched_switch(void)
{
        ...
        struct kthread * old_thread = curthr;
        curthr = ktqueue_dequeue(&kt_runq); // switch to next
        curproc = curthr->kt_proc;
        context_switch(&old_thread->kt_ctx, &curthr->kt_ctx);
        // NOT_YET_IMPLEMENTED("PROCS: sched_switch");
        intr_setipl(old_IPL); // back before interupt
        dbg(DBG_PRINT, "(GRADING1C)\n");
}
...
{       
        ...
        intr_setipl(old_IPL); // back to init 
        dbg(DBG_PRINT, "(GRADING1C)\n");
