...
int pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        ...
        while (1)
        {
                /* Find the page if it is already resident. */
                pf = pframe_get_resident(o, pagenum);
                if (pf != NULL)
                {
                        /* If the page is busy, we wait for it to become unbusy and then
                           try again. */
                        ...
                        /* Page is resident and not busy. */
                        *result = pf;

                        KASSERT(NULL != *result); /* on successful return, must return a
                                                     valid pframe object */
                        KASSERT(
                            !pframe_is_busy(*result)); /*  the returned pframe object must
                                                          not be in the "busy" state */
                        dbg(DBG_PRINT, "(GRADING3A 1.a)\n");
                        dbg(DBG_PRINT, "(GRADING3A)\n");
                        return 0;
                }

                // /* Page not resident, check if we need to run pageoutd. */
                // if (pageoutd_needed())
                // {
                //         pageoutd_wakeup();
                ...
                /* Fill the page. */
                int fill_res = pframe_fill(pf);
                if (fill_res < 0)
                {
                        /* If there is an error filling the page, free it. */
                        pframe_free(pf);
                ...
                *result = pf;
                ...
                KASSERT(NULL != *result);          /* on successful return, must return a valid
                                                      pframe object */
                KASSERT(!pframe_is_busy(*result)); /*  the returned pframe object must
                                                      not be in the "busy" state */
                dbg(DBG_PRINT, "(GRADING3A 1.a)\n");
                dbg(DBG_PRINT, "(GRADING3A)\n");
...
void pframe_pin(pframe_t *pf)
{
        ...
        KASSERT(
            !pframe_is_free(pf)); /* can only pin a pframe object that's "in-use" */
        KASSERT(pf->pf_pincount >=
                0); /* the pin-count of a pframe object cannot be negative */
        dbg(DBG_PRINT, "(GRADING3A 1.b)\n");

        /* Increase the pin count on this page */
        pf->pf_pincount++;

        /* If this is the first pin, move the page from allocated to pinned list */
        if (pf->pf_pincount == 1)
        {
                /* Remove the page from allocated list */
                list_remove(&pf->pf_link);

                /* Decrease the number of allocated pages */
                nallocated--;

                /* Add the page to pinned list */
                list_insert_tail(&pinned_list, &pf->pf_link);

                /* Increase the number of pinned pages */
                npinned++;
                dbg(DBG_PRINT, "(GRADING3A)\n");
        }
        dbg(DBG_PRINT, "(GRADING3A)\n");
...
void pframe_unpin(pframe_t *pf)
{
        ...
        KASSERT(
            !pframe_is_free(pf)); /* can only pin a pframe object that's "in-use" */
        KASSERT(pf->pf_pincount >
                0); /* the pin-count of a pframe object must be positive */
        dbg(DBG_PRINT, "(GRADING3A 1.c)\n");

        // Decrease the pin count
        pf->pf_pincount--;

        // If pin count is zero
        if (pf->pf_pincount == 0)
        {
                ...
                // Increase the number of allocated pages
                nallocated++;
                dbg(DBG_PRINT, "(GRADING3A)\n");
        }
        dbg(DBG_PRINT, "(GRADING3A)\n");
